...
## Как получить полный код проекта одним архивом

Если нужно отдать весь рабочий проект «целиком» (без `.git` и `node_modules`), используйте:

```bash
npm run export:full-code
```

Архив появится в папке `dist/` с именем вида `taxiweb-full-source-YYYYMMDD-HHMMSS.tar.gz`.
